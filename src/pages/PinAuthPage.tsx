  ...
  useEffect(() => {
    const fetchPinUsers = async () => {
      try {
        ...
        if (error) {
          console.error('Error fetching PIN users:', error);
          toast.error('Failed to load PIN users');
          return;
        }
          ...
          <div className="space-y-2">
            ...
            <Select value={selectedUserId} onValueChange={setSelectedUserId} disabled={loadingUsers}>
              <SelectTrigger className="w-full">
                <SelectValue placeholder={loadingUsers ? "Loading users..." : "Choose your profile"} />
              </SelectTrigger>
              <SelectContent>
                {pinUsers.map((user) => (
