...
const orderFieldOptions = [
  ...
  { value: 'customer.whatsappLink', label: 'Customer WhatsApp Link' },
...
const DeliveryLabelSettings = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      await dataService.settings.updateDeliveryLabelTemplate(template);
      toast.success("Delivery label template updated successfully");
    } catch (error) {
      console.error("Failed to update delivery label template", error);
      toast.error("Failed to update delivery label template");
    }
  };

  const handleTemplateChange = (key: keyof DeliveryLabelTemplate, value: string) => {
    if (!template) return;
    setTemplate({ ...template, [key]: value });
  };

  const handleSectionChange = (sectionId: string, key: keyof DeliveryLabelSection, value: any) => {
    if (!template) return;
  ...
  const addSection = () => {
    ...
    const newSection: DeliveryLabelSection = {
  ...
  // Generate the field editor component
  const renderFieldEditor = (section: DeliveryLabelSection, field: DeliveryLabelField) => {
    // The field editor component is similar to the PrintSettings component
    // This implements the form fields for editing a delivery label field
    return (
      <div key={field.id} className="space-y-3 border p-3 rounded-md bg-background">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-2">
            ...
            <Select 
              value={field.type} 
              onValueChange={(value) => handleFieldChange(section.id, field.id, 'type', value as DeliveryLabelFieldType)}
            >
              ...
              <div className="space-y-2 col-span-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="customer.whatsappLink">Customer WhatsApp Link</SelectItem>
    ...
    <TabsContent value="delivery-label">
      <Card>
        <CardHeader>
          <CardTitle>Delivery Label Layout</CardTitle>
          <CardDescription>
            Customize the layout of your delivery label (4x6 inches). The label will be printed in portrait orientation.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">Configure general settings for the delivery label</p>
              </div>
              <div className="flex gap-2">
                <Button
                  variant="outline"
            ...
            <div className="space-y-4 pt-2">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-medium">Label Sections</h3>
