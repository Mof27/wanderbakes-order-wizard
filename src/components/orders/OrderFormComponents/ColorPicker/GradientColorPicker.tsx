      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
            ...
            return <div key={`top-${color.value}`} className={`h-12 rounded-md cursor-pointer border-2 flex items-center justify-center ${value[0] === color.value ? "border-primary" : "border-transparent"}`} style={{
        ...
        {colorCount === 3 && <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
              {baseColors.map(color => {
            const textColor = getContrastTextColor(color.value);
            return <div key={`middle-${color.value}`} className={`h-12 rounded-md cursor-pointer border-2 flex items-center justify-center ${value[1] === color.value ? "border-primary" : "border-transparent"}`} style={{
              backgroundColor: color.value
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
            {baseColors.map(color => {
            const textColor = getContrastTextColor(color.value);
            return <div key={`bottom-${color.value}`} className={`h-12 rounded-md cursor-pointer border-2 flex items-center justify-center ${value[colorCount === 2 ? 1 : 2] === color.value ? "border-primary" : "border-transparent"}`} style={{
              backgroundColor: color.value
